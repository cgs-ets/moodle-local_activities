import { Alert, Anchor, Avatar, Button, Modal, Table } from "@mantine/core"
...
export function Conflicts() {
  ...
  return (
    !!conflicts.length
    ? <div className="flex border-x border-b border-gray-300/70">
        <Alert className="p-0 m-0 inline-block shadow-none rounded-none w-full" variant="light" color="orange">
          <Button c="black" onClick={() => conflictsOpened ? hideConflicts() : showConflicts()} variant="transparent" className="px-2 font-normal" rightSection={conflictsOpened ? <IconChevronUp className="size-5"/> : <IconChevronRight className="size-5"/>}>
            {conflicts.length} {conflicts.length > 1 ? "activities overlap" : "activity overlaps" } with this activity
          </Button>
        </Alert>
        <Modal
        ...
        >
          <Alert className="p-0 shadow-none rounded-none" variant="light" color="orange">
            <div>
              <Table>
                <Table.Tbody>
                  <Table.Tr>
